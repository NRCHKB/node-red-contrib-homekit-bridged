module.exports = function(RED) {
    ...
    const MdnsUtils = require("./utils/MdnsUtils.js")(RED);
    const ServerUtils = require("./utils/ServerUtils.js")(RED);
    ...
    const init = function (config) {
        ...
        debug(RED._("homekit.bridge.setting_name_to"), config.bridgeName);
        ...
        debug(RED._("homekit.bridge.creating"), this.name, bridgeUUID);

        let bridge = new Bridge(this.name, bridgeUUID);

        this.publish = function () {
            debug(RED._("homekit.bridge.publishing"), self.name, self.pinCode, bridge.bridgedAccessories.length);
        ...
        bridge.on("identify", function(paired, callback) {
            if (paired) {
                debug(RED._("homekit.bridge.identify_called_on_paired"), self.name);
            } else {
                debug(RED._("homekit.bridge.identify_called_on_unpaired"), self.name);
            }
