module.exports = function(RED) {
    ...
    const init = function (config) {
        ...
        if (config.customMdnsConfig) {
            ...
            if (typeof config.mdnsMulticast === "boolean")
                this.mdnsConfig.multicast = config.mdnsMulticast;

            if(isIP(config.mdnsInterface))
                this.mdnsConfig.interface = config.mdnsInterface;

            if (typeof config.mdnsPort === "number")
                this.mdnsConfig.port = config.mdnsPort;

            if(isIP(config.mdnsIp))
                this.mdnsConfig.ip = config.mdnsIp;

            if (typeof config.mdnsTtl === "number")
                this.mdnsConfig.ttl = config.mdnsTtl;

            if (typeof config.mdnsLoopback === "boolean")
                this.mdnsConfig.loopback = config.mdnsLoopback;

            if (typeof config.mdnsReuseAddr === "boolean")
