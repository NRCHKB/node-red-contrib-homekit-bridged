...
Camera.prototype.configure = function(config, cameraControlService) {
    ...
    if (cameraControlService) {
        ...
        console.warn("Camera reconfigure on the fly is not yet supported. " +
...
Camera.prototype.handleSnapshotRequest = function (request, callback) {
    ...
    ffmpeg.on("close", function (code) {
        ...
        if (this.cameraNode) {
            const topic = this.cameraNode.topic ? this.cameraNode.topic : this.cameraNode.topic_in;
            msg = {payload: {}, name: this.cameraNode.name, topic: topic};
        }

        if (this.uploader !== "disabled" && !this.cameraNode) {
            console.error("Node required for camera to send snapshot!");
            ...
            fs.writeFile(filePath, imageBuffer, function(err) {
                if (err) {
                    return console.error("Camera snapshot file failed to save due to: " + err);
                }

                msg.payload["cameraSnapshot"] = filePath;
                debug("Camera snapshot file has been saved! Sending to output... " + JSON.stringify(msg));
                self.cameraNode.send([null, msg]);
            });
        } else if (this.uploader === "content") {
            msg.payload["cameraSnapshot"] = imageBuffer;
            debug("Sending camera snapshot buffer to output..." + JSON.stringify(msg));
