module.exports = function(RED) {
    ...
    const configure = function (node, config) {
        ...
        const node.service = ServiceUtils.getOrCreate(accessory, {
            name: node.name,
            UUID: subtypeUUID,
            serviceName: node.serviceName
        }, parentService);

        node.characteristicProperties = CharacteristicUtils.loadCharacteristicProperties(node.service, config);
        CharacteristicUtils.addCharacteristicListener(node.service, node);

        publishTimers = BridgeUtils.delayedPublish(node, publishTimers);
        
//        node.service = service;
        
        node.supported = CharacteristicUtils.getSupported(node.service);
