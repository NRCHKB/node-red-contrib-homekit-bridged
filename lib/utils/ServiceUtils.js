    ...
    const getOrCreate = function (accessory, serviceInformation, parentService) {
        ...
        if (!service) {
            ...
            if (serviceInformation.serviceName === "CameraControl") {
                if (serviceInformation.config.cameraConfigSource) {
                    debug("Configuring Camera Source");
                    accessory.configureCameraSource(new CameraSource(newService, serviceInformation.config));
                    service = newService;
                } else {
        ...
        if (parentService) {
            if (serviceInformation.name === "CameraControl") {
                node.error("CameraControl shouldn't be Linked Service!!!");
            }
