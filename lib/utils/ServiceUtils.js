module.exports = function(RED, node) {
    ...
    const onInput = function (msg) {
        if (msg.hasOwnProperty("payload")) {
            ...
            if (type !== "object") {
                node.warn(RED._("homekit.bridge.invalid_payload_type"), type);
                return;
            }
        } else {
            node.warn(RED._("homekit.bridge.invalid_message_payload_missing"));
            return;
        }

        const topic = node.topic ? node.topic : node.name;
        if (node.filter === true && msg.topic !== topic) {
            debug(RED._("homekit.bridge.msg_topic_not_match"));
            return;
        }
        ...
        Object.keys(msg.payload).map(function (key, index) {
            if (node.supported.write.indexOf(key) < 0) {
                ...
                node.warn(RED._("homekit.characteristic.cannot_be_written"), key, node.supported.write.join(", "));
            } else {
    ...
    const getOrCreate = function (accessory, serviceInformation, parentService) {
        ...
        debug(RED._("homekit.service.looking_for"), serviceInformation.UUID);
        ...
        for (let i in accessory.services) {
            ...
            if (
                ...
                debug(RED._("homekit.service.found"));
        ...
        if (!service) {
            debug(RED._("homekit.service.not_find_adding"));
            service = accessory.addService(newService);
        }

        if (parentService) {
            debug(RED._("homekit.service.linking_to_parent"));
