module.exports = function(node) {
    ...
    function addListener(node, characteristic){
        if (characteristic.props.perms.indexOf("ev") > -1) {
                characteristic.on('change', function(oldValue, newValue, context) {
                    const topic = node.topic ? node.topic : node.topic_in;
                    const msg = {payload:{}, hap: context, name: node.name, topic: topic};
                    msg.payload[this.displayName] = {"oldValue":oldValue, "newValue":newValue};
                    
                    node.status({
                        fill: "yellow",
                        shape: "dot",
                        text: this.displayName + ": " + value.newValue
                    });
...
//                msg.payload[this.displayName] = this.value;
                ...
                node.status({
                    fill: "yellow",
                    shape: "dot",
                    text: this.displayName + ": " + this.Value
                });
