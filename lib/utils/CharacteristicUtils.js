module.exports = function(node) {
    ...
    function addListener(node, characteristic){
        if (characteristic.props.perms.indexOf("ev") > -1) {
                characteristic.on('change', function(oldVal, newVal, context) {
                    const topic = node.topic ? node.topic : node.topic_in;
                    const msg = {payload:{}, hap: {oldValue:oldVal, newValue:newVal, context: context}, name: node.name, topic: topic};
                    msg.payload[this.displayName] = {oldValue:oldVal, newValue:newVal};
                    
                    node.status({
                        fill: "yellow",
                        shape: "dot",
                        text: "CHANGE "+ this.displayName + ": " + oldVal + " -> " + newVal
                    });
        ...
        if (characteristic.props.perms.indexOf("pr") > -1) {
            characteristic.on('get', function(callback, context) {
                ...
                node.status({
                    fill: "green",
                    shape: "dot",
                    text: "GET " + this.displayName + ":" + this.value
                });
        ...
        if (characteristic.props.perms.indexOf("pw") > -1) {
            characteristic.on('set', function(newVal, callback, context) {
                ...
                node.status({
                    fill: "red",
                    shape: "dot",
                    text: "SET " + this.displayName + ": " + newVal
                });
