...
type SessionInfo = {
    ...
    timeout: NodeJS.Timeout // releases pending resources if START never arrives
    ...
    implements CameraStreamingDelegate, CameraRecordingDelegate
{
    ...
    handleSnapshotRequest(
        ...
        const ffmpeg = spawn(ffmpegPath, splitCommandLine(ffmpegCommand), {
            env: process.env,
        })
    ...
    handleStreamRequest(
        ...
        switch (request.type) {
            case StreamRequestTypes.START: {
                ...
                const additional =
                    cfg?.cameraConfigAdditionalCommandLine?.trim()
                ...
                const settleCallback = (error?: Error) => {
                    if (callbackSettled) {
                        return
                    }
                ...
                ffmpegVideo.stderr.on('data', (data: Buffer) => {
                    ...
                    if (!started) {
                        ...
                        settleCallback() // remember to execute a callback once set up
                    }

                    if (
                        this.ffmpegDebugOutput ||
                        this.config?.cameraConfigDebug
                    ) {
